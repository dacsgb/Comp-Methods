...
class PlotCanvas(FigureCanvas):
    ...
    def plotit(self,lin = None, quad = None, cub = None):
        self.figure.clf()

        if lin!= None and quad!= None and cub != None:
            ...
            ax.plot(lin.xcalc,lin.ycalc, label ='First order')
            ax.plot(quad.xcalc,quad.ycalc, label ='Second order')
            ax.plot(cub.xcalc,cub.ycalc, label ='Third order')
...
class main_window(QDialog):
    ...
    def assign_widgets(self):
        ...
        self.ui.all_but.clicked.connect(self.fit_check)
        self.ui.Exit_but.clicked.connect(self.ExitApp)
    
    def fit_check(self):
        if self.ui.all_but.isChecked():
            self.fit = 'all'
    ...
    def Calculate(self):
        ...
        if self.fit == 'all':
            self.graph.plotit(lin = self.LSF_lin, quad = self.LSF_quad, cub = self.LSF_cub)
            self.ui.EQ_out.setText('')
